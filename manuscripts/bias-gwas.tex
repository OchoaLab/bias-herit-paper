...
\begin{itemize}
\item Add MOR (mean-of-ratios) version of standard kinship to empirical tests.
  (Currently only ROM (ratio-of-means) is being tested, it'd be good to have both.
  See \cref{eq:kinship_std_rom} and \cref{eq:kinship_std_mor}; both are implemented in my package popkinsuppl.)
\item Add another kinship estimator (Weir-Goudet: WG) to empirical tests.
  \textbf{Disclosure: WG is a competitor, so I'm hoping to show that this is worse.}
  (See \cref{eq:wg,eq:wg_lim}, which will be implemented in popkinsuppl shortly.)
\item If WG also performs like the true kinship matrix
  \begin{itemize}
  ...
  \end{itemize}
\item If WG doesn't perform like the true kinship matrix
  \begin{itemize}
  \item Plot precision-recall curves for methods, to show if WG actually performs worse or about the same (on average?)
  \item Bonus: prove theoretically that this will not be an equivalent model (looks hard, don't know where to start)
  \end{itemize}
\item Switch from admixture-simulated genotypes to real genotypes (1000 Genomes or HDGP), because there are huge differences between LMM and PCA in real data (but not in admixture simulation).
  This also simplifies the methods.
  \begin{itemize}
