...
if(G==20) {# calculate pedigree-based kinships
  ped <- pedigree(id=famG$id, dadid=famG$pat, momid=famG$mat, sex=famG$sex)
  
  kin_ped<-kinship(ped)
  
  ## subset data to the last generation
  kin_ped0<-kin_ped[ids[[20]],ids[[20]]]
  }
...
if(G==20){# pedigree kinship
  write_grm('pedigree', 2*kin_ped0) }
