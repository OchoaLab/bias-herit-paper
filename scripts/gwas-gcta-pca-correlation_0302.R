...
# GWAS
obj_gcta_default_lim <- gas_lmm_gcta(
    gcta_bin,
    name_out,
    name_grm = name_kinship_gcta_lim,
    m_loci = m_loci,
    threads = threads
)
...
message( "GCTA (std mor kinship)" )
# write the kinship matrix to a file
name_kinship_std_mor <- paste0(name_out, '.kinship_std_mor')
write_grm(
    name_kinship_std_mor,
    kinship = 2 * kinship_std_mor,
    M = M,
    fam = plink_data$fam,
    verbose = FALSE
)
# GWAS
obj_gcta_std_mor <- gas_lmm_gcta(
    gcta_bin,
    name_out,
    name_grm = name_kinship_std_mor,
    m_loci = m_loci,
    threads = threads
)
...
# cleanup
delete_files_gcta(name_out) # GAS table
delete_files_grm(name_kinship_std_mor) # GRM
...
# GCTA with limit of biased "standard" kinship estimate
message( "GCTA (wg kinship)" )
# write the kinship matrix to a file
name_kinship_wg <- paste0(name_out, '.kinship_wg')
write_grm(
    name_kinship_wg,
    kinship = 2 * kinship_wg,
    M = M,
    fam = plink_data$fam,
    verbose = FALSE
)
# GWAS
obj_gcta_wg <- gas_lmm_gcta(
    gcta_bin,
    name_out,
    name_grm = name_kinship_wg,
    m_loci = m_loci,
    threads = threads
)
## obj_gcta_std_lim$pvals
...
# cleanup
delete_files_gcta(name_out) # GAS table
delete_files_grm(name_kinship_wg) # GRM

# GCTA with limit of biased "standard" kinship estimate
message( "GCTA (wg lim kinship)" )
# write the kinship matrix to a file
name_kinship_wg_lim <- paste0(name_out, '.kinship_wg_lim')
write_grm(
    name_kinship_wg_lim,
    kinship = 2 * kinship_wg_lim,
    M = M,
    fam = plink_data$fam,
    verbose = FALSE
)
# GWAS
obj_gcta_wg_lim <- gas_lmm_gcta(
    gcta_bin,
    name_out,
    name_grm = name_kinship_wg_lim,
    m_loci = m_loci,
    threads = threads
)
## obj_gcta_std_lim$pvals
...
# cleanup
delete_files_gcta(name_out) # GAS table
delete_files_grm(name_kinship_wg_lim) # GRM
...
gcta_pval<-data.frame(obj_gcta_default$pvals,obj_gcta_default_lim$pvals,
                      obj_gcta_true$pvals,obj_gcta_popkin$pvals,
                      obj_gcta_wg$pvals,obj_gcta_wg_lim$pvals,
                      obj_gcta_std$pvals,obj_gcta_std_mor$pvals,
                      obj_gcta_std_lim$pvals)

colnames(gcta_pval)<-c("gcta_default","gcta_default_lim",
                       "gcta_true","gcta_popkin",
                       "gcta_wg","gcta_wg_lim",
                       "gcta_std","gcta_std_mor",
                       "gcta_std_lim")
...
gcta_beta_hat<-data.frame(obj_gcta_default$beta_hat,obj_gcta_default_lim$beta_hat,
                      obj_gcta_true$beta_hat,obj_gcta_popkin$beta_hat,
                      obj_gcta_wg$beta_hat,obj_gcta_wg_lim$beta_hat,
                      obj_gcta_std$beta_hat,obj_gcta_std_mor$beta_hat,
                      obj_gcta_std_lim$beta_hat)

colnames(gcta_beta_hat)<-c("gcta_default","gcta_default_lim",
                           "gcta_true","gcta_popkin",
                           "gcta_wg","gcta_wg_lim",
                           "gcta_std","gcta_std_mor",
                           "gcta_std_lim")
...
message( "PCA (gcta default)" )
eigenvectors_gcta <- kinship_to_evd( kinship_gcta )
obj_pca_default <- gas_pca_optim(X, trait, eigenvectors_gcta[, indexes_true ])
## obj_pca_popkin$pvals
## obj_pca_popkin$beta_hat

message( "PCA (gcta default limit)" )
eigenvectors_gcta_lim <- kinship_to_evd( kinship_gcta_lim )
obj_pca_default_lim <- gas_pca_optim(X, trait, eigenvectors_gcta_lim[, indexes_true ])
## obj_pca_popkin$pvals
## obj_pca_popkin$beta_hat

message( "PCA (wg kinship)" )
eigenvectors_wg <- kinship_to_evd( kinship_wg )
obj_pca_wg <- gas_pca_optim(X, trait, eigenvectors_wg[, indexes_std ])

message( "PCA (wg kinship limit)" )
eigenvectors_wg_lim <- kinship_to_evd( kinship_wg_lim )
obj_pca_wg_lim <- gas_pca_optim(X, trait, eigenvectors_wg_lim[, indexes_std ])
...
message( "PCA (std mor kinship)" )
eigenvectors_std_mor <- kinship_to_evd( kinship_std_mor )
obj_pca_std_mor <- gas_pca_optim(X, trait, eigenvectors_std_mor[, indexes_std ])
...
######
pca_pval<-data.frame(obj_pca_default$pvals,obj_pca_default_lim$pvals,
                      obj_pca_true$pvals,obj_pca_popkin$pvals,
                      obj_pca_wg$pvals,obj_pca_wg_lim$pvals,
                      obj_pca_std$pvals,obj_pca_std_mor$pvals,
                      obj_pca_std_lim$pvals)

colnames(pca_pval)<-c("pca_default","pca_default_lim",
                       "pca_true","pca_popkin",
                       "pca_wg","pca_wg_lim",
                       "pca_std","pca_std_mor",
                       "pca_std_lim")
...
pca_beta_hat<-data.frame(obj_pca_default$beta_hat,obj_pca_default_lim$beta_hat,
                          obj_pca_true$beta_hat,obj_pca_popkin$beta_hat,
                          obj_pca_wg$beta_hat,obj_pca_wg_lim$beta_hat,
                          obj_pca_std$beta_hat,obj_pca_std_mor$beta_hat,
                          obj_pca_std_lim$beta_hat)

colnames(pca_beta_hat)<-c("pca_default","pca_default_lim",
                           "pca_true","pca_popkin",
                           "pca_wg","pca_wg_lim",
                           "pca_std","pca_std_mor",
                           "pca_std_lim")
...
#write.table(all_pval,"all_pval_0228.txt",row.names = FALSE  )
...
#write.table(all_beta_hat,"all_beta_hat_0228.txt",row.names = FALSE  )


############ read file

setwd( 'D:/3.Duke/research/alex_ochoa/1.reverse_regression/coding/mycode/true-vs-biased-kinship-gwas' )

all_pval<-read.table("all_pval_10000.txt",header = TRUE)

all_beta_hat<-read.table("all_beta_hat_10000.txt",header = TRUE)
...
# pheatmap(
#     all_pval, scale = "row", 
#     clustering_distance_cols = as.dist(1 - cor(all_pval)),
#     clustering_distance_rows = as.dist(1 - cor(all_pval))
# )
# 
# 
# heatmap(as.matrix(cor(all_pval)), scale = "row")
# 
# heatmap(as.matrix(cor(all_beta_hat)), scale = "row")
